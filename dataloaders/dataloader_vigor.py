...
class VIGORDataset(Dataset):
    def __init__(self, root='/home/ziminxia/Work/datasets/VIGOR', label_root=None, split=None, train=None, random_orientation=None, transform=None, first_run=None):
        ...
        self.city_list = self._get_city_list() #加载城市列表
        self.sat_list, self.sat_index_dict = self._load_satellite_data() #加载航拍数据 24255 ,由于图像名称较长，将其一一对应到索引,sat的索引
        self.grd_list, self.label, self.delta, self.sat_cover_dict = self._load_ground_data() #加载地面数据 14107 (14107,4) (14107,4,2) 10752 加载了4个但是实验的时候只选取第一个
        #sat_cover_dict是一个字典，key是sat的索引，value是一个list，list里面是所有与该sat对应的grd的索引,一个航拍图像是多个（<=2）地面图像的正样本
        self.data_size = len(self.grd_list) # 14107
        ...
        for city in self.city_list:
            ...
            with open(label_fname, 'r') as file:
                for line in file:
                    ...
                    label_list.append(label)#sate图片的序号
                    delta_list.append(delta) #row_offset, col_offset
                    
                    if label[0] not in sat_cover_dict:
                        sat_cover_dict[label[0]] = [idx]
                    else:
                        sat_cover_dict[label[0]].append(idx) #一个航拍图像是多个（<=2）地面图像的正样本
                    idx += 1
    ...
    def _get_label_file(self, city):
        if self.split == 'samearea':
            return os.path.join(self.root, self.label_root, city, 'same_area_balanced_train__corrected.txt' if self.train else 'same_area_balanced_test__corrected.txt')
            #return os.path.join(self.root, self.label_root, city, 'same_area_balanced_train__corrected.txt' if self.train else 'test_debug.txt')
        return os.path.join(self.root, self.label_root, city, 'pano_label_balanced__corrected.txt')
    ...
    def __getitem__(self, idx):
        ...
        if self.random_orientation:#如果需要随机方向
            if self.first_run:#如果是第一次运行则随机旋转一个角度
                rotation = np.random.uniform(-180/360, 180/360) 
            else: #如果不是第一次则看是否预测了朝向
                if self.ori_pred[idx] == 'None':#没有预测则随机旋转一个角度
                    rotation = np.random.uniform(-180/360, 180/360) 
                else:#预测了则根据预测值调整旋转角度
        ...
        sat, row_offset, col_offset = self._load_satellite_image(idx)#位置的gt是读出来的

        gt_loc = torch.tensor([[-row_offset, col_offset]]) # 位置gt
        r = torch.tensor([[np.cos(yaw), -np.sin(yaw)], [np.sin(yaw), np.cos(yaw)]]).to(torch.float32) # 旋转矩阵gt r
    ...
    def _load_image(self, path, default_size):
        try:
            ...
            img = Image.new('RGB', default_size)#创建空白图像
        return img

    def _load_satellite_image(self, idx):
        row_offset, col_offset = self.delta[idx, 0] 
        # 每一个里面有4个偏移，其中1个positive以及3个semi-positive,相对于中心的偏移
        ...
        # Normalize offsets to the resized image size
